...
Как можно видеть - мы запрашиваем и отсылаем возрастающие акноледжи. При этом мы не всегда получаем, что видно по количествую 
...
#### Демонстрация работы
Вся передача файла вынесена в отдельные функции, так что не важно от кого кому передавать
...
Можете удостовериться, что каждый раз при отправке я вызываю checksum и создавая и проверяя  контрольные суммы
...
Заметим, что средняя пропускная способность - это объём данных делённый на время. Средний размер окна: $\frac{3W}{4}$ при таком увеличении. Время на среднюю величину мы понимаем, как изменяется - тогда одно можно выразить через другое
...
$\bullet 1$ Заметим, что из данных неравств понятно, что наше окно будет сначала размера 1 (как всегда), после третьего пакета оно увеличится до 4х. Из первого неравенства, мы понимаем, что нам успеет прийти подтверждение за 4ый пакет - значит размер окна увеличится до 8 - а значит остальное мы просто доотправим. И того $3 RTT + \frac{S}{R} \cdot (2 + 4 + 8)$

$\bullet 2$ Теперь мы успеем после 3его пакета получить акнолежд за второй, после 7го получить за 4ый. Дальше всё уже доотправиться (7 + 8 = 15), между каждыми этапами мы ждали $RTT$, т.е. in total: $4 RTT + \frac{S}{R} \cdot (1 + 1 + 1 + 8)$  

$\bullet 3$ Т.к. RTT мал - то ответы будут успевать к нам прилетать и мы будем последовательно отправлять пакеты. Т.е. нужно  $2 RTT + \frac{S}{R} \cdot 15$  
...
Как было сказано выше - на общение с сервером необходимо установление соединения и отправка пакетов - т.е. 4 $RTT$, только теперь это $RTT_{FE}$. Сервера могут обменяться данными между собой за $RTT_{BE}$ и время обработки никуда не девается - т.е. ровно столько, сколько сказано в $(1)$
