...
<img src="images/1_2.png" width=1400 />

<img src="images/1_3.png" width=1400 />
...
1. Выберите первое ICMP-сообщение эхо-запроса, отправленное вашим компьютером, и
   ...
   Вообще вопрос имеет место для подвоха. Я хоть и сижу на винде, но делаю всё через wsl, через его терминал всё выполняю (т.е. у меня локально хостится на компе свой сервер, из которого я вижу мир). И тут я могу говорить про ip внутри этой сети, про ip внутри общажного интернета. На скрине (для меня пока не понятно почему), но виден ip моего компа от общажного интернета (проверил через настройки интернета, там такое же значение)
...
3. Сколько байт в IP-заголовке? Сколько байт приходится на полезную нагрузку IP-дейтаграммы?
   ...
   На заголовок IP - 20 байт
   Длина данных - 36 байт
...
<img src="images/1_4.png" width=1000 />

   Каждый раз меняется только: Identification
   Header checksum disabled - так что она всегда 0 и не меняется 
   ...
   2. Какие поля не меняются? Какие поля должны оставаться неизменными? Какие
      ...
      Вообще мне начинает казаться, что нужно смотреть не те сообщения, которые отсылал наш компьютер, а те, которые он получал (мы же будто используем этот протокол, чтоб посмотреть ответы при истечении TTL). Но делаю, как сказано

      Не меняются поля с параметрами: Version, header len, DSCP, ECN, total length, Flags, Fragment offset, Protocol, Source address

      TTL и Destination address будто бы должны меняться, но когда мы смотрим на IMCP, которые отправлялись моим компом - там они неизменные
...
5. Какое значение содержится в поле Identification (Идентификация), а какое – в поле TTL
   ...
   <img src="images/1_5.png" width=1000 />
...
7. Найдите серии откликов ICMP, в которых содержатся сообщения об истечении
   ...
   <img src="images/1_6.png" width=1400 />

   Identification = 16445
   TTL = 65
...
8. **Фрагментация**. Остановите захват пакетов в Wireshark. Измените размер пакета
   ...
   Если я ставлю размер отправки больший 1500 - то он не справляется отправлять, если ставлю 1500 - то он в одном пакете всё отправляет. Не знаю почему, но он не справляется разбить на дейтаграммы (или нужно искать не через udp, но тогда не понимаю, где)

   2. Какие поля IP-заголовка изменяются в разных фрагментах?
      
   В прошлом пункте ничего понять не получилось. Но я пока копался в файлах, пойманных wireshark, узнал, что айфон моего соседа посылает какую-то инфу про него. Будте осторожнее, беря айфоны
...
#### Демонстрация работы
   ...
   <img src="images/2_1.png" width=1400 />

   <img src="images/2_2.png" width=1400 />
...
<img src="images/go-back-n.png" width=1400 />

#### Демонстрация работы

За исключением логирования и с шириной окна = 1, данное задание уже делал в lab. Думаю, этих частичных баллов мне будет достаточно
