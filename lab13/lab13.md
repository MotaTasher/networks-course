...
1. Т.е. он должен отправить, а остальные нет, чтоб не произошло коллизий и чтоб до этого он не отправлял. Шанс этого:
$p \cdot (1 - p) ^ 3 (1 - p) ^ 3 \cdot (1 - p) ^ 4$

2. $3 p (1 - p)^3 \cdot (1 - p)^3$ - есть три непересекающихся случая - когда у одного получилось, а у других нет. Причём нужно домножить, что это именно первый раз

3. Вероятность успешной передачи в данный такт - $4p(1-p)^4$, причём до этого было 2 неудачи, значит в итоге имеем: $4p(1-p)^4 \cdot (1 - 4p(1-p)^4)^2$
...
Будем оценивать эффективность в рамках одной сессии. Когда кто-то отвечает, то будет передано $Q$ бит информации за $d_{\text{опрос}} + \frac{Q}{R}$ временип. В итоге:
$\frac{Q}{d_{\text{опрос}} + \frac{Q}{R}}$
