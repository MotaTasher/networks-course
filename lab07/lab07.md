...
<img src="images/1_1.png" width=900 />

<img src="images/1_2.png" width=900 />

<img src="images/1_3.png" width=900 />
...
1. Какой IP-адрес и номер порта TCP использует ваш компьютер (отправитель), передающий
   ...
   Наш IP - 10.110.186.173, и порт 40302, после - 40814
...
2. Каков IP-адрес у сервера gaia.cs.umass.edu? Каковы номера портов для отправки и приема
   ...
   dst port - 80 (вначале мы что-то отправили на 443, а после - на 80 слали. Причём браузер дал мне предупреждение, что соединение стало небезопасным, мб это как раз от того, что ssl бы на 443, а после перешли на 80)

2. Какой порядковый номер у SYN TCP-сегмента, который используется для установления
   TCP-соединения между компьютером клиента и сервером gaia.cs.umass.edu? Как
   определяется, что это именно SYN-сегмент?

<img src="images/1_4.png" width=900 />

   Номер - нулевой 

3. Какой порядковый номер у SYNACK-сегмента, отправленного сервером gaia.cs.umass.edu
...
<img src="images/1_5.png" width=900 />

   Номер опять нулевой, поле - 0x012. Он поставил единичку на биты отвечающие за "SYN", "ACK"

4. Какой порядковый номер у TCP-сегмента, содержащего команду POST протокола HTTP?
...
<img src="images/1_6.png" width=900 />

Номер - первый. Нашёл PUSH, а дальше в посмотрел его исходный код и в нём найден был POST

5. Рассмотрите TCP-сегмент, содержащий команду POST протокола HTTP, как первый TCP-сегмент 
...
-  | Номера сегментов | Время отправки | Получен ACK пакет                 | RTT       |
   |------------------|----------------|-----------------------------------|-----------|
   | 1                | 4.356972       | 4.509130100                       | 0.1521581 |
   | 657              | 4.357099       | (не было)                         | 0.1520312 |
   | 2020             | 4.357108       | (не было)                         | 0.1520222 |
   | 3383             | 4.357120       | 4.509130200                       | 0.1520102 |
   | 4746             | 4.357126       | (не было)                         | 0.1520042 |
   | 6109             | 4.359078       | 4.509130260                       | 0.1500522 |
1. Чему равна пропускная способность (количество байтов, передаваемых в единицу
   времени) для этого TCP-соединения? Объясните, как вы получили это значение.
   
   Возьмём window size и поделим на средний RTT:

   <img src="images/1_6.png" width=900 />

   $\frac{64256}{0.15} \sim 428373$ байт в секунду $\sim 0.4$ мб/с
...
#### Скрин
   ...
   <img src="images/2_1.png" width=900 />
...
Клиент делает всё тоже самое - считает пинги, отправляя теперь свой индекс и время. Если время слишком сильно отличается - мы считаем чела убитым (показываю момент рождения и момент смерти по каждому индексу. Можно было б накрутить более крутую систему генерации индекса, но рандома хватает)
...
Мы можем сравнить, сколько времени уходит на чистую доставку данных против того, сколько мы в реальности тратим. Это тоже должно быть 98%


$$\frac{windows\_size\cdot\frac{L}{R}}{RTT + \frac{L}{R}}\geq 0.98$$

$$ windows\_size\geq 0.98\cdot(1 + \frac{R}{L} \cdot RTT)$$

$$windows\_size\geq 3.4$$
